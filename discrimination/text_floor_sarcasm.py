...
DEFAULT_OUTPUT = PACKAGE_ROOT / "runs" / "sarcasm_discrimination" / "_text_floor" / "results.jsonl"
...
def prompt_fields() -> tuple[str, str]:
    ...
    block = prompts["sarcasm_discrimination"]
    system = "\n".join(block["system"])
    user = "\n".join(block["user"])
    return system, user
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="sarcasm-discrimination-text-floor")
