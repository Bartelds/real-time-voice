...
DEFAULT_OUTPUT = PACKAGE_ROOT / "runs" / "fear_discrimination" / "_text_floor" / "results.jsonl"
...
def prompt_fields() -> tuple[str, str]:
    ...
    block = prompts["fear_discrimination"]
    system = "\n".join(block["system"])
    user = "\n".join(block["user"])
    return system, user
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="fear-discrimination-text-floor")
